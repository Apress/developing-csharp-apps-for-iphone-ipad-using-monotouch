...
namespace Apress.Screens.ADONET
{
	public partial class BasicOperations : UITableViewController
	{
		protected List<string> _people = new List<string>();
		...
		protected void Initialize ()
		{
			...
			using (var cmd = connection.CreateCommand ())
			{
				...
				using (var reader = cmd.ExecuteReader ())
				{
					...
					while (reader.Read ()) { this._people.Add(reader[1] + " " + reader[2]); }
				}
			...
			this._tableSource = new BasicOperations.TableSource(this._people);
	    ...
	    protected SqliteConnection GetConnection(string dbName)
	    {
			...
			if (needToCreate) { SqliteConnection.CreateFile (db); }
	        ...
	        if (needToCreate) { this.CreateDBSchema(conn); }
		...
		protected class TableSource : UITableViewSource
		{
			...
			public TableSource(List<string> items) : base() { this._items = items; }
			
			public override int NumberOfSections (UITableView tableView) { return 1; }
			
			public override int RowsInSection (UITableView tableview, int section) { return this._items.Count; }
			
			public override UITableViewCell GetCell (UITableView tableView, NSIndexPath indexPath)
			{
				UITableViewCell cell;
				cell = tableView.DequeueReusableCell("item");
				if(cell == null) { cell = new UITableViewCell(UITableViewCellStyle.Default, "item"); }
				cell.TextLabel.Text = this._items[indexPath.Row];
				return cell;
			}
